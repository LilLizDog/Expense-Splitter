...
def get_history(
    ...
    received_rows = recv_resp.data or [] # this is a list
    ...
    paid_rows = paid_resp.data or []

    # Apply optional filters so the frontend does not have to change
    # Group filter
    if group:
        g = group.lower()
        received_rows = [
            r for r in received_rows 
            if g in (r.get("group_name") or "").lower()
        ]
        paid_rows = [
            p for p in paid_rows 
            if g in (p.get("group_name") or "").lower()]
    ...
    if person:
        ...
        received_rows = [
            r for r in received_rows 
            if p in (r.get("from_name") or "").lower()
        ]
...
def get_history_groups():
    ...
    if recv_resp.data is None or paid_resp.data is None:
    ...
    recv_groups = {
        r["group_name"] 
        for r in (recv_resp.data or []) 
        if r.get("group_name")
    }
    paid_groups = {
        p["group_name"] 
        for p in (paid_resp.data or []) 
        if p.get("group_name")
    }
